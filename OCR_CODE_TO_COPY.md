...
<body>
  ...
  <div class="card">
    ...
    <p style="font-size:12px; color:var(--muted); margin:0 0 10px;">Take a photo of an existing ID — the agent will read it and fill matching fields automatically. You still confirm everything before submission.</p>
    <input type="file" accept="image/*" capture="environment" id="scanInput" style="display:none;" onchange="handleScanFile(event)">
    <button onclick="document.getElementById('scanInput').click()" style="background:var(--accent); color:#fff; padding:8px 14px; font-size:12px;">📷 Scan Document</button>
...
function registerTools() {
  const toolDefs = [
    ...
    {
      name: "scan_document",
      description: "Extracts identity fields (name, DOB, PAN, Aadhaar number) from a photo of an existing ID document, and fills matching fields on the active form automatically. Human still reviews and approves before submission.",
      inputSchema: {
        type: "object",
        properties: {
          form_id: { type: "string" },
...
// ---------- Document Scan (OCR auto-fill) ----------
function extractFieldsFromText(text) {
  const extracted = {};
  let remaining = text.replace(/\s+/g, ' ');

  // Date of Birth: DD/MM/YYYY or DD-MM-YYYY — extract and remove first, so its digits
  // can't be mistaken for part of an Aadhaar number below.
  const dobMatch = remaining.match(/\b\d{2}[\/\-]\d{2}[\/\-]\d{4}\b/);
  if (dobMatch) {
    extracted.dob = dobMatch[0];
    remaining = remaining.replace(dobMatch[0], ' ');
  }

  // PAN: 5 letters + 4 digits + 1 letter (e.g. ABCDE1234F)
  const panMatch = remaining.match(/\b[A-Z]{5}[0-9]{4}[A-Z]\b/);
  if (panMatch) {
    extracted.pan_number = panMatch[0];
    remaining = remaining.replace(panMatch[0], ' ');
  }

  // Aadhaar: 12 digits, often spaced in groups of 4 (checked after DOB/PAN removed)
  const aadhaarMatch = remaining.match(/\b\d{4}\s?\d{4}\s?\d{4}\b/);
  if (aadhaarMatch) {
    extracted.aadhaar_number = aadhaarMatch[0].replace(/\s/g, '');
    remaining = remaining.replace(aadhaarMatch[0], ' ');
  }

  // Name: text after "Name" label, non-greedy, stopping at the next label/digit
...
async function handleScanFile(event) {
  ...
  previewEl.innerHTML = `<img src="${imgUrl}" style="max-width:100%; border-radius:8px; border:1px solid var(--border);">`;
  ...
  try {
    ...
    statusEl.textContent = 'Text extracted, matching fields...';

    const extracted = extractFieldsFromText(text);
    const foundCount = Object.keys(extracted).length;

    if (foundCount === 0) {
      statusEl.textContent = 'No recognizable fields found. Try a clearer photo, or fill manually / by voice.';
      return;
    }

    // Ensure a form session exists
    if (!state) {
      const r = await registeredTools.start_form.execute({ application_type: currentAppType });
      // start_form already sets `state` internally via closure
    }

    for (const [fieldName, value] of Object.entries(extracted)) {
      // Only fill fields that exist on the currently selected form
      if (!(fieldName in state.fields)) continue;
      await registeredTools.fill_field.execute({ form_id: state.form_id, field_name: fieldName, value: value });
    }

    statusEl.textContent = `Auto-filled ${foundCount} field(s) from scan. Review below before approving.`;
  } catch (err) {
    statusEl.textContent = 'Scan failed: ' + err.message + '. Please fill manually.';
  }
}
